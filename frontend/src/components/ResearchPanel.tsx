...
export default function ResearchPanel({ ticker, assumptions = null }: { ticker: string; assumptions?: Record<string, number> | null }) {
  ...
  return <section className="research-room relative overflow-hidden p-6 lg:p-9">
    ...
    {!result && !error && <Spinner label="Gemini is analyzing the company evidence…" />}
    ...
    {result?.status === "needs_setup" && <p className="text-sm text-on-surface-variant">Gemini research is not connected yet. Once configured, this panel will explain the company’s financial position, price trends, analyst views, and clinical catalysts.</p>}
    {result?.status === "ready" && <div className="space-y-4 text-sm leading-relaxed">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="rounded border border-outline-variant px-2 py-1 capitalize text-primary">{result.outlook.replaceAll("_", " ")} outlook</span>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {([['Supporting factors', result.drivers], ['Risks and counterpoints', result.risks]] as const).map(([title, points]) => <div key={title}>
          <h3 className="mb-2 font-semibold">{title}</h3>
          <ul className="space-y-3">{points.map((point, i) => <li key={i}>{point.text}<EvidenceRefs ticker={ticker} ids={point.evidence_ids} /></li>)}</ul>
      ...
      {result.tensions.length > 0 && <div className="rounded-2xl border border-outline-variant bg-surface-container-low p-5">
        <h3 className="font-semibold">Model tensions</h3>
        <p className="mt-1 text-xs text-on-surface-variant">Where the DCF and signal outputs disagree with the rest of the evidence. These are challenges to the models, not a recommendation.</p>
        <ul className="mt-3 space-y-3">{result.tensions.map((point, i) => <li key={i}>{point.text}<EvidenceRefs ticker={ticker} ids={point.evidence_ids} /></li>)}</ul>
      </div>}
      ...
      <details className="border-t border-outline-variant pt-3"><summary className="cursor-pointer text-primary">Inspect evidence used ({result.evidence.length} records)</summary>
        <div className="mt-3 space-y-2">{result.evidence.map(item => <details id={`research-${ticker}-${item.id}`} key={item.id} className="rounded border border-outline-variant p-2"><summary className="cursor-pointer">[{item.id}] {item.label}</summary><pre className="mt-2 whitespace-pre-wrap break-words text-xs text-on-surface-variant">{JSON.stringify(item.data, null, 2)}</pre></details>)}</div>
      </details>
      <p className="text-xs text-on-surface-variant">AI-generated interpretation of the displayed evidence. It can make mistakes; evidence references do not guarantee factual accuracy. The DCF and signal shown here remain separate deterministic calculations — Gemini examines them, it does not produce the recommendation.</p>
    </div>}
    <form className="mt-6 flex flex-col gap-3 rounded-2xl bg-surface-container p-2 pl-4 sm:flex-row sm:items-center" onSubmit={event => { event.preventDefault(); setSubmitted(question.trim()); setAttempt(value => value + 1); }}>
      <Icon name="auto_awesome" className="hidden text-secondary sm:inline-block" /><input aria-label="Ask Plutus a research question" maxLength={1000} value={question} onChange={event => setQuestion(event.target.value)} placeholder={`Ask Plutus about ${ticker} financials, risks, or clinical catalysts…`} className="min-w-0 flex-1 bg-transparent py-2 text-sm outline-none" />
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Explore</span>{["What are the main risks?", "Assess the cash position", "Explain the analyst outlook"].map(prompt => <button key={prompt} onClick={() => setQuestion(prompt)} className="rounded-full bg-surface-container-low px-3 py-1.5 text-xs text-on-surface-variant hover:bg-surface-container">{prompt}</button>)}</div>
  </section>;
}
