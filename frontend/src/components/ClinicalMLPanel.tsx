...
export default function ClinicalMLPanel({ ticker, onIngest }: { ticker: string; onIngest?: () => void }) {
  ...
  return <section className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-6" aria-label="Clinical success model">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><h2 className="text-lg font-semibold">Clinical phase advancement</h2>
        <p className="mt-1 text-sm text-on-surface-variant">Protocol language and study design inform the estimated chance of Phase 1→2 or Phase 2→3 progression.</p></div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {data.trials.slice(0, visible).map(trial => <article key={trial.nct_id} className="rounded-xl border border-outline-variant p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="text-xs text-on-surface-variant">{trial.phase.replaceAll("PHASE", "Phase ").replaceAll("+", " / ")}{trial.next_phase ? ` → ${trial.next_phase.replace("PHASE", "Phase ")}` : ""}</span>
          </div>
          ...
          <div className="mt-3 border-t border-outline-variant pt-3">
            {trial.probability !== null ? <p className="text-lg font-semibold">{(trial.probability * 100).toFixed(0)}% <span className="text-xs font-normal text-on-surface-variant">estimated next-phase chance · {trial.evidence_reliability} model reliability</span></p>
            ...
            {trial.drivers.length > 0 && <details className="mt-2 text-xs text-on-surface-variant"><summary className="cursor-pointer">Model factors</summary>
              <p className="mt-2">Protocol words and design features associated with past advancement. They do not establish causes. Training vocabulary covers {(trial.text_vocabulary_coverage * 100).toFixed(0)}% of this trial's terms.</p>
